class L1Config {
    ...
    public L1CHARCFGS:{ [key: number]: L1CharConfig };
    public L1LevelCFGS: { [key: number]: number[] };

    private static _instance:L1Config;
    public static get Ins():L1Config {
        if(this._instance){
            return this._instance;
        }
        this._instance = new L1Config();
        return this._instance;
    };

    private constructor() {
        this.L1CHARCFGS = this.GetCHARCFGS();
        this.L1LevelCFGS = this.GetL1LevelCFGS();
    }

    private GetCHARCFGS(): { [key: number]: L1CharConfig } {

        let L1HARMCFGS: { [key: string]: L1HarmConfig } = {
            // 模版
    ...
    private GetL1LevelCFGS(): { [key: number]: number[] } {
        return {
            0: [1, 1],
            1: [1, 1, 1],
            2: [1, 1, 2, 2]
